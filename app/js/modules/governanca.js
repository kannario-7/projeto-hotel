...
export function renderGovernanca(){
  ...
  el.innerHTML='<div class="page-header"><div><h2>Limpeza</h2><p>Quartos aguardando limpeza e manutenção</p></div></div>';
  ...
  el.innerHTML+='<div class="cards-row">'+
    ...
    '<div class="stat-card"><h3>Em manutenção</h3><div class="value" style="color:var(--neg)">'+manut.length+'</div></div>'+
    '<div class="stat-card"><h3>Disponíveis</h3><div class="value" style="color:var(--pos)">'+quartos.filter(function(q){return q.status==="disponivel";}).length+'</div></div>'+
    '</div>';
  ...
  if(!fila.length){
    ...
    el.innerHTML+='<div style="overflow-x:auto"><table><tr><th>Quarto</th><th>Tipo</th><th>Na fila</th><th>Responsável</th><th>Ações</th></tr>'+
    fila.map(function(q){
      var t=tq.find(function(x){return x.id===q.tipoQuartoId;});
      var opts='<option value="">Sem responsável</option>'+camareiras.map(function(c){return '<option value="'+c.id+'"'+(q.limpezaResponsavel===c.id?' selected':'')+'>'+esc(c.nome)+'</option>';}).join('');
      return '<tr>'+
    ...
    if(!camareiras.length){
      el.innerHTML+='<p style="color:var(--text-mute);font-size:13px;margin-top:8px">Dica: cadastre funcionários com cargo "Camareira" em Funcionários para atribuí-los à limpeza.</p>';
    }
  }

  // Manutencao (visao rapida)
  if(manut.length){
    el.innerHTML+='<h3 style="margin:22px 0 10px;color:var(--text)">Em manutenção</h3><div style="overflow-x:auto"><table><tr><th>Quarto</th><th>Tipo</th><th>Ações</th></tr>'+
    manut.map(function(q){var t=tq.find(function(x){return x.id===q.tipoQuartoId;});return '<tr><td><b>Apto '+esc(q.numero)+'</b></td><td>'+esc(t?t.nome:"Quarto")+'</td><td><button class="btn btn-sm btn-success" onclick="marcarQuartoLimpo(\''+q.id+'\')">Liberar (disponível)</button></td></tr>';}).join('')+'</table></div>';
  }
}
...
export function atribuirCamareira(quartoId, funcId){
  ...
  auditar("limpeza.atribuir", funcId?("Atribuiu "+nomeFuncionario(funcId)+" ao Apto "+(q?q.numero:"")):("Removeu responsável do Apto "+(q?q.numero:"")));
  st(funcId?("Responsável: "+nomeFuncionario(funcId)):"Responsável removido.","success");
  renderGovernanca();
}
...
export function marcarQuartoLimpo(quartoId){
  ...
  auditar("limpeza.concluir","Apto "+q.numero+" liberado (limpo/disponível)");
  st("Apto "+q.numero+" pronto e disponível!","success");
  renderGovernanca();
}
