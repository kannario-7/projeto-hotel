// Módulo: Check-in
import { esc, fmtD, td } from "../utils.js";
...
if(ocupados.length){el.innerHTML+='<h3 style="margin-bottom:12px;color:var(--text)">Em Andamento ('+ocupados.length+')</h3><table><tr><th>Hospede</th><th>Quarto</th><th>Check-in</th><th>Check-out</th></tr>'+
ocupados.map(function(r){var h=St.fi("h",r.hospedeId),q=St.fi("q",r.quartoId);return'<tr><td>'+(h?esc(h.nome):"-")+'</td><td>'+(q?esc("Apto "+q.numero):"-")+'</td><td>'+fmtD(r.dataCheckin)+'</td><td>'+fmtD(r.dataCheckout)+'</td></tr>'}).join('')+'</table>';}
