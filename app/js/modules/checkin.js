...
el.innerHTML='<div class="page-header"><div><h2>Check-in</h2><p>Realizar check-in de hóspedes</p></div></div>';

if(ocupados.length){el.innerHTML+='<h3 style="margin-bottom:12px;color:var(--text)">Em Andamento ('+ocupados.length+')</h3><table><tr><th>Hóspede</th><th>Quarto</th><th>Check-in</th><th>Check-out</th><th>Consumo</th><th>Ações</th></tr>'+
...
return'<tr><td>'+(h?esc(h.nome):"-")+'</td><td>'+(q?esc("Apto "+q.numero):"-")+'</td><td>'+fmtD(r.dataCheckin)+'</td><td>'+fmtD(r.dataCheckout)+'</td><td>'+(totalConsumo?fmtC(totalConsumo):"-")+(consumos.length?' <small style="color:var(--text-mute)">('+consumos.length+')</small>':'')+'</td><td><button class="btn btn-sm btn-primary" onclick="showNovoConsumo(\''+r.id+'\')">+ Lançar consumo</button></td></tr>'}).join('')+'</table>';}

if(reservas.length){el.innerHTML+='<h3 style="margin-bottom:12px;color:var(--text)">Reservas para Check-in</h3><table><tr><th>Hóspede</th><th>Quarto</th><th>Check-in</th><th>Check-out</th><th>Documento</th><th>Ação</th></tr>'+
...
export function realizarCheckin(id){confirmar({titulo:"Confirmar check-in?",msg:"O hóspede será registrado e o quarto marcado como ocupado.",okLabel:"Confirmar check-in",tipo:"info"},function(){
...
auditar("checkin.realizar","Check-in de "+(h?h.nome:"hóspede")+(q?(" no Apto "+q.numero):""));
