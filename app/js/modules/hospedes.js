...
el.innerHTML='<div class="page-header"><div><h2>Hóspedes</h2><p>Cadastro de hóspedes</p></div><div class="page-header-actions"><button class="btn btn-primary" onclick="showNovoHospede()">+ Novo Hóspede</button></div></div>';
el.innerHTML+='<div id="hospedesList">'+buildHospedesTable(hospedes,reservas)+'</div>';}

function buildHospedesTable(hospedes,reservas){if(!hospedes.length)return'<p style="padding:24px;text-align:center;color:var(--text-mute)">Nenhum hóspede cadastrado.</p>';
return'<table><tr><th>Nome</th><th>Documento</th><th>Telefone</th><th>Email</th><th>Total Reservas</th><th>Ações</th></tr>'+
hospedes.map(function(h){var total=reservas.filter(function(r){return r.hospedeId===h.id}).length;return'<tr><td>'+esc(h.nome)+'</td><td>'+esc(h.documento||"-")+'</td><td>'+esc(h.telefone||"-")+'</td><td>'+esc(h.email||"-")+'</td><td>'+total+'</td><td style="white-space:nowrap"><button class="btn btn-sm btn-primary" onclick="editarHospede(\''+h.id+'\')">Editar</button> <button class="btn btn-sm btn-secondary" onclick="exportarHospede(\''+h.id+'\')" title="Baixar os dados deste hóspede (LGPD)">Dados</button> <button class="btn btn-sm btn-secondary" onclick="anonimizarHospede(\''+h.id+'\')" title="Remover dados pessoais preservando o histórico (LGPD)">Anonimizar</button> <button class="btn btn-sm btn-danger" onclick="excluirHospede(\''+h.id+'\')">Excluir</button></td></tr>'}).join('')+'</table>'}

export function showNovoHospede(){sm("Novo Hóspede",formHospede(null),'<button class="btn btn-secondary" onclick="closeModal()">Cancelar</button><button class="btn btn-primary" onclick="salvarHospede()">Salvar</button>')}
export function editarHospede(id){sm("Editar Hóspede",formHospede(id),'<button class="btn btn-secondary" onclick="closeModal()">Cancelar</button><button class="btn btn-primary" onclick="salvarHospede(\''+id+'\')">Salvar</button>')}

function formHospede(id){var h=id?St.fi("h",id):null;
return'<div class="form-grid">'+
'<div class="form-group"><label>Nome Completo *</label><input type="text" id="hfNome" value="'+(h?esc(h.nome):'')+'" placeholder="Nome do hóspede"></div>'+
...
'<div class="form-group"><label>Endereço</label><input type="text" id="hfEnd" value="'+(h?esc(h.endereco||""):'')+'" placeholder="Rua, bairro, cidade/UF"></div>'+
'<div class="form-group" style="grid-column:1/-1"><label>Observações</label><textarea id="hfObs" rows="2">'+(h?esc(h.observacoes||""):'')+'</textarea></div>'+
'<div class="form-group" style="grid-column:1/-1"><label class="pay-opt" style="border:none;padding:6px 0"><input type="checkbox" id="hfConsent"'+((h&&h.consentimentoEm)?' checked':'')+'><span class="pay-box"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span><span class="pay-label">O hóspede autoriza o tratamento dos seus dados para fins da hospedagem (LGPD).</span></label>'+
...
export function mascHospedeDoc(el){
  ...
  if(dig.length===11){msg.textContent=isValidCPF(dig)?"CPF válido.":"CPF inválido.";msg.style.color=isValidCPF(dig)?"var(--pos)":"var(--neg)";}
...
export async function buscarCepHospede(){
  ...
  if(!e){msg.textContent="CEP não encontrado.";return;}
  end.value=e;msg.textContent="Endereço preenchido pelo CEP.";
}
...
if(!n||!n.value.trim())return st("Nome é obrigatório.","error"),false;
...
if(!cons||!cons.checked)return st("É necessário o consentimento do hóspede (LGPD) para cadastrar.","error"),false;
...
if(id){St.up("h",id,dados);st("Hóspede atualizado!","success")}
else{St.in("h",dados);st("Hóspede cadastrado!","success")}
cm();renderHospedes()}

export function excluirHospede(id){confirmar({titulo:"Excluir hóspede?",msg:"Esta ação não poderá ser desfeita.",okLabel:"Sim, excluir",tipo:"danger"},function(){St.rm("h",id);st("Hóspede excluído.","warning");renderHospedes()})}
...
export function exportarHospede(id){
  ...
  st("Dados do hóspede exportados.","success");
}

// LGPD: anonimizar (remove dados pessoais preservando o registro/estatistica de reservas)
export function anonimizarHospede(id){
  var h=St.fi("h",id);if(!h)return;
  confirmar({titulo:"Anonimizar este hóspede?",msg:"Nome, documento, telefone, e-mail e endereço serão apagados permanentemente. O histórico de reservas é mantido, mas sem identificar a pessoa. Não pode ser desfeito.",okLabel:"Sim, anonimizar",tipo:"danger"},function(){
    St.up("h",id,{nome:"[Anonimizado]",documento:"",telefone:"",email:"",endereco:"",observacoes:""});
    st("Dados pessoais do hóspede anonimizados.","success");
    renderHospedes();
  });
}
