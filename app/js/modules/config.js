...
var config=St.gc();
el.innerHTML='<div class="page-header"><div><h2>Configurações</h2><p>Configurar dados do hotel e sistema</p></div></div>'+
'<div class="tabs">'+
...
'<div class="tab" onclick="mudarConfigTab(this,\'us\')">Usuários</div>'+
...
var ACAO_LABEL={
  ...
  "usuario.desativar":{t:"Usuário desativado",c:"var(--neg)"},
  "usuario.ativar":{t:"Usuário ativado",c:"var(--pos)"},
  ...
  "tarifa.excluir":{t:"Tarifa excluída",c:"var(--neg)"}
};
...
export async function renderAtividades(){
  ...
  if(!itens.length){ box.innerHTML='<div class="form-container"><p style="color:var(--text-mute)">Nenhuma atividade registrada ainda. As ações importantes (cancelamentos, check-in/out, caixa) aparecerão aqui.</p></div>'; return; }
  ...
  box.innerHTML='<div class="form-container"><h3 style="margin-bottom:6px;color:var(--text)">Atividades recentes</h3>'+
    '<p style="color:var(--text-mute);font-size:13px;margin-bottom:14px">Registro de quem fez o que no sistema (últimas 200 ações).</p>'+
    '<div style="overflow-x:auto"><table><tr><th>Ação</th><th>Detalhe</th><th>Usuário</th><th>Quando</th></tr>'+linhas+'</table></div></div>';
}
...
'<div class="form-group"><label>Razão Social</label><input type="text" id="cfgRazao" value="'+esc(c.hrazao||"")+'"></div>'+
...
'</div>'+
'<h3 style="margin:22px 0 14px;color:var(--text)">Localização</h3><div class="form-grid">'+
...
'<div class="form-group"><label>Endereço (rua/av.)</label><input type="text" id="cfgEnd" value="'+esc(c.hend||"")+'"></div>'+
'<div class="form-group"><label>Número</label><input type="text" id="cfgNum" value="'+esc(c.hnum||"")+'"></div>'+
...
'</div>'+
'<h3 style="margin:22px 0 14px;color:var(--text)">Operação</h3><div class="form-grid">'+
'<div class="form-group"><label>Horário Check-in</label><input type="time" id="cfgHci" value="'+esc(c.hci||"14:00")+'"></div>'+
'<div class="form-group"><label>Horário Check-out</label><input type="time" id="cfgHco" value="'+esc(c.hco||"12:00")+'"></div>'+
'<div class="form-group"><label>Taxa de Serviço (%)</label><input type="number" id="cfgTax" value="'+(c.tax||10)+'" min="0" max="100"></div>'+
...
function secaoReservasOnline(c){
  ...
  return '<div class="form-container"><h3 style="margin-bottom:6px;color:var(--text)">Reservas online (página pública)</h3>'+
    '<p style="color:var(--text-mute);font-size:13px;margin-bottom:14px">Ative uma página pública onde qualquer pessoa consulta disponibilidade e solicita uma reserva. As solicitações chegam como <b>pendentes</b> em Reservas para você confirmar.</p>'+
    '<label class="pay-opt" style="border:none;padding:6px 4px;margin-bottom:8px"><input type="checkbox" id="cfgReservasPub"'+(ativo?' checked':'')+' onchange="toggleReservasPublicas(this.checked)"><span class="pay-box"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span><span class="pay-label">Aceitar reservas pela página pública</span></label>'+
    (ativo && c.slug
...
export async function toggleReservasPublicas(ativo){
  ...
  if(!res.ok){ st("Não foi possível alterar. Tente novamente.","error"); var c=document.getElementById("cfgReservasPub"); if(c)c.checked=!ativo; return; }
...
St.sc(cfg);st("Configurações salvas!","success");}
...
export async function buscarCep(){
  ...
  if(cep.length!==8){msg.textContent="Informe um CEP com 8 dígitos.";return;}
  ...
  try{
    ...
    msg.textContent="Endereço preenchido pelo CEP.";
...
export async function buscarDocumento(){
  ...
  try{
    ...
    if(!r.ok){msg.textContent="CNPJ não encontrado.";return;}
    var d=await r.json();
...
if(tq.length){html+='<table><tr><th>Nome</th><th>Capacidade</th><th>Preço Diária</th><th>Ações</th></tr>'+
...
'<div class="form-group"><label>Preço da Diária (R$)</label><input type="number" id="tqfPreco" step="0.01" min="0"></div>'+
...
'<div class="form-group"><label>Preço da Diária (R$)</label><input type="number" id="tqfPreco" step="0.01" min="0" value="'+(t.precoDiaria/100).toFixed(2)+'"></div>'+
...
if(!n||!n.value.trim())return st("Nome obrigatório.","error"),false;
...
export function excluirTipoQuarto(id){
  ...
  if(usando.length){st("Não é possível excluir: "+usando.length+" quarto(s) usam o tipo \""+t.nome+"\". Altere ou exclua esses quartos antes.","error");return;}
  confirmar({titulo:"Excluir o tipo \""+t.nome+"\"?",msg:"O tipo de quarto será removido. Reservas antigas que o referenciam são mantidas.",okLabel:"Sim, excluir",tipo:"danger"},function(){
    St.up("tq",id,{ativo:false});
    st("Tipo de quarto excluído.","warning");
    renderConfig();
  });
}

// ================= ABA TARIFAS (temporada / fim de semana) =================
var DIAS_SEM=[["0","Dom"],["1","Seg"],["2","Ter"],["3","Qua"],["4","Qui"],["5","Sex"],["6","Sáb"]];
...
function formConfigTarifas(tq){
  ...
  var html='<div class="form-container"><h3 style="margin-bottom:6px;color:var(--text)">Tarifas por temporada e dia da semana</h3>'+
    '<p style="color:var(--text-mute);font-size:13px;margin-bottom:16px">Defina preços diferentes por período (alta/baixa temporada, feriados) ou por dias da semana. Sem regra, vale o preço padrão do tipo de quarto.</p>';
  if(!tq.length){ html+='<p style="color:var(--text-mute)">Cadastre tipos de quarto antes de criar tarifas.</p></div>'; return html; }
  if(tarifas.length){
    html+='<table><tr><th>Tipo</th><th>Regra</th><th>Quando</th><th>Preço</th><th>Ações</th></tr>'+
    ...
    html+='<p style="color:var(--text-mute)">Nenhuma tarifa especial cadastrada. Vale o preço padrão de cada tipo de quarto.</p>';
  }
  ...
  html+='<div class="form-container"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:6px"><h3 style="color:var(--text)">Assistente de preços</h3><button class="btn btn-sm btn-secondary" onclick="renderSugestoesTarifa()">Atualizar</button></div>'+
    '<p style="color:var(--text-mute);font-size:13px;margin-bottom:14px">Analisa a ocupação dos próximos dias e <b>sugere</b> ajustes de diária. Nada é alterado automaticamente: você revisa e cria a tarifa se quiser.</p>'+
    '<div id="tarifasSugestoes"><p style="color:var(--text-mute)">Clique em "Atualizar" para gerar as sugestões.</p></div></div>';
  return html;
}
...
export function renderSugestoesTarifa(){
  ...
  if(!tipos.filter(function(t){return t.ativo!==false;}).length || !quartos.filter(function(q){return q.ativo!==false;}).length){
    alvo.innerHTML='<p style="color:var(--text-mute)">Cadastre tipos de quarto e quartos para receber sugestões.</p>'; return;
  }
  var sugestoes=sugerirTarifas(quartos, reservas, tipos, { hoje: td() });
  if(!sugestoes.length){
    alvo.innerHTML='<p style="color:var(--text-mute)">Sem sugestões no momento. A ocupação dos próximos dias está equilibrada.</p>'; return;
  }
  ...
  var linhas=sugestoes.slice(0,40).map(function(s){
    ...
    var badge=s.motivo==="alta"
      ...
      : '<span class="badge badge-info">Última hora '+pct+'%</span>';
  ...
  alvo.innerHTML='<table><tr><th>Tipo</th><th>Data</th><th>Situação</th><th>Diária</th><th>Ação</th></tr>'+linhas+'</table>';
}
...
function formTarifa(t){
  ...
  return '<div class="form-group"><label>Tipo de quarto *</label><select id="tfTipo">'+
      tipos.map(function(x){return '<option value="'+x.id+'"'+(t&&t.tipoQuartoId===x.id?' selected':'')+'>'+esc(x.nome)+' (padrão '+fmtC(x.precoDiaria)+')</option>';}).join('')+
    '</select></div>'+
    ...
    '<div class="form-group"><label>Tipo de regra *</label><select id="tfRegra" onchange="tarifaToggleRegra()">'+
      '<option value="periodo"'+(regra==="periodo"?' selected':'')+'>Por período (datas)</option>'+
    ...
    '<div id="tfBoxPeriodo" style="display:'+(regra==="periodo"?"block":"none")+'"><div class="form-grid">'+
      '<div class="form-group"><label>Início</label><input type="date" id="tfInicio" value="'+(t&&t.dataInicio?t.dataInicio:"")+'"></div>'+
    ...
    '</div></div>'+
    '<div class="form-group"><label>Preço da diária nesta regra (R$) *</label><input type="number" id="tfPreco" step="0.01" min="0" value="'+(t&&t.preco?(t.preco/100).toFixed(2):"")+'"></div>'+
...
export function salvarTarifa(id){
  ...
  if(!p||p<=0)return st("Informe um preço válido.","error"),false;
  ...
  if(regra.value==="periodo"){
    ...
    if(!ci||!ci.value||!cf||!cf.value)return st("Informe início e fim da temporada.","error"),false;
...
export function excluirTarifa(id){
  var t=St.fi("tf",id);if(!t)return;
  confirmar({titulo:"Excluir tarifa?",msg:"Esta regra de preço será removida. As diárias voltam a usar o preço padrão (ou outra regra).",okLabel:"Sim, excluir",tipo:"danger"},function(){
    St.rm("tf",id);
    auditar("tarifa.excluir","Excluiu tarifa de "+nomeTipo(t.tipoQuartoId));
    st("Tarifa excluída.","warning");
    renderConfig();
  });
}
...
function rotuloForma(p){
  var mapa={dinheiro:"Dinheiro",debito:"Débito",credito:"Crédito",pix:"Pix",boleto:"Boleto",cheque:"Cheque",transferencia:"Transferência",link:"Link de pagamento",voucher:"Voucher / Cortesia"};
...
'<p style="color:var(--text-mute);margin-bottom:16px">Selecione as formas aceitas pelo hotel. Você também pode adicionar formas personalizadas.</p>'+
'<div class="pay-list">'+
...
'</div>'+
'<div class="pay-add"><input type="text" id="cfgNovaForma" placeholder="Adicionar forma (ex: PicPay, Vale-refeição)" onkeydown="if(event.key===\'Enter\'){event.preventDefault();adicionarFormaPagamento()}"><button type="button" class="btn btn-secondary" onclick="adicionarFormaPagamento()">+ Adicionar</button></div>'+
...
'<p style="color:var(--text-mute);font-size:12px;margin-top:8px">Apaga todos os dados operacionais deste hotel (reservas, hóspedes, quartos, etc.). Protegido por senha-mestra do dono.</p></div>'}
...
export function adicionarFormaPagamento(){
  ...
  if(atuais.indexOf(nome)>=0 || FORMAS_PADRAO.indexOf(nome)>=0)return st("Essa forma já existe.","warning"),false;
  atuais.push(nome);
  cfg.pm=atuais;
  St.sc(cfg);
...
export function restaurarDados(){
  sm("Restaurar Dados do Hotel",
    '<div class="alert alert-warning" style="margin-bottom:14px"><span class="aico-wrap">⚠️</span><span>Esta ação <b>apaga todos os dados operacionais</b> deste hotel: reservas, hóspedes, quartos, tipos, serviços, consumos e pagamentos. Não pode ser desfeita.</span></div>'+
...
export async function confirmarRestauracao(){
  ...
  try{
    ...
    if(error){
      ...
      return st("Não foi possível restaurar: "+error.message,"error");
    }
