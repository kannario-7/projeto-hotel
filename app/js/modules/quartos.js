...
(q.status==="disponivel"?('<button class="btn btn-sm btn-primary" onclick="showManutencaoQuarto(\''+q.id+'\')">Manutenção</button>'):'')+
...
(q.icalToken?'<button class="btn btn-sm btn-secondary" onclick="mostrarLinkIcal(\''+q.id+'\')" title="Sincronizar datas com Airbnb/Booking">Calendário</button>':'')+
'<button class="btn btn-sm btn-danger" onclick="excluirQuarto(\''+q.id+'\')">Excluir</button></div></div>'});
html+='</div>'});
el.innerHTML+=html;}

export function showManutencaoQuarto(id){St.up("q",id,{status:"manutencao"});st("Quarto em manutenção.","warning");renderQuartos()}
export function liberarQuarto(id){St.up("q",id,{status:"disponivel"});st("Quarto disponível.","success");renderQuartos()}
...
export function mostrarLinkIcal(id){
  ...
  if(!q.icalToken)return st("Este quarto ainda não tem calendário. Rode a atualização do banco (schema-28).","error");
  ...
  sm("Calendário do Apto "+esc(q.numero),
    '<h4 style="color:var(--text);margin:0 0 6px">1. Exportar (bloquear no Airbnb/Booking)</h4>'+
    '<p style="color:var(--text-dim);font-size:13px;margin-bottom:8px">Cole este link no Airbnb/Booking (Importar calendário). Eles bloqueiam as datas ocupadas aqui.</p>'+
    ...
    '<h4 style="color:var(--text);margin:0 0 6px">2. Importar (bloquear aqui o que foi vendido lá)</h4>'+
    '<p style="color:var(--text-dim);font-size:13px;margin-bottom:8px">Cole aqui os links de calendário (.ics) do Airbnb/Booking <b>deste quarto</b>. Ao sincronizar, as datas vendidas lá ficam bloqueadas aqui.</p>'+
    ...
    '<p style="color:var(--text-mute);font-size:12px;margin-top:10px">Sincroniza apenas datas ocupadas (não preço/hóspede). Atualização periódica.</p>',
...
export async function salvarIcalUrls(id){
  ...
  if(!res.ok)return st("Não foi possível salvar os links.","error"),false;
  st("Links de calendário salvos.","success");
  return true;
}
export async function sincronizarIcal(id){
  ...
  try{
    ...
    if(data && data.ok){
      ...
      st("Não foi possível sincronizar. Verifique os links.","error");
...
export function excluirQuarto(id){
  ...
  if(ativas.length){st("Não é possível excluir: o Apto "+q.numero+" tem "+ativas.length+" reserva(s) ativa(s). Cancele ou finalize antes.","error");return;}
  if(q.status==="ocupado"){st("Não é possível excluir: o Apto "+q.numero+" está ocupado.","error");return;}
  confirmar({titulo:"Excluir o Apto "+q.numero+"?",msg:"O quarto será removido da lista. O histórico de reservas antigas é mantido.",okLabel:"Sim, excluir",tipo:"danger"},function(){
    // Exclusao logica: marca inativo (preserva historico de reservas que referenciam o quarto)
    St.up("q",id,{ativo:false});
    st("Quarto excluído.","warning");
    renderQuartos();
  });
}
...
'<div class="form-group"><label>Número *</label><input type="text" id="qfNum" value="'+(q?esc(q.numero):'')+'"></div>'+
...
'<div class="form-group"><label>Status</label><select id="qfStatus"><option value="disponivel"'+(q&&q.status==="disponivel"?' selected':'')+'>Disponível</option><option value="manutencao"'+(q&&q.status==="manutencao"?' selected':'')+'>Manutenção</option><option value="limpeza"'+(q&&q.status==="limpeza"?' selected':'')+'>Limpeza</option></select></div>'+
'</div>';}
...
if(!n||!n.value.trim())return st("Número obrigatório.","error"),false;
