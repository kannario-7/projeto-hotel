...
function avisoCadastroHTML(){
  ...
  if(!c.hend || !(""+c.hend).trim()) faltando.push("endereço");
...
function avisoPlanoHTML(){
  ...
  if(d < 0) return '<div class="alert alert-danger">'+aIco("danger")+'<span>Seu plano venceu. Renove para não perder o acesso. <a href="'+wa+'" target="_blank" rel="noopener" style="color:var(--accent-2);text-decoration:underline">Renovar</a></span></div>';
...
if(quartos.length===0 && reservas.length===0 && hospedes.length===0){
  el.innerHTML='<div class="page-header"><div><h2>Bem-vindo ao HospedaPrime!</h2><p>Vamos preparar seu hotel em 3 passos rápidos'+infoTurno+'</p></div></div>'+
  '<div class="welcome-steps">'+
  '<div class="wstep"><div class="wstep-num">1</div><div class="wstep-body"><b>Cadastre os tipos de quarto</b><p>Standard, Suíte, etc. com capacidade e diária.</p><button class="btn btn-primary btn-sm" onclick="navTo(\'#cg\')">Ir para Configurações</button></div></div>'+
  '<div class="wstep"><div class="wstep-num">2</div><div class="wstep-body"><b>Cadastre seus quartos</b><p>Número, andar e tipo de cada quarto do hotel.</p><button class="btn btn-primary btn-sm" onclick="navTo(\'#q\')">Ir para Quartos</button></div></div>'+
  '<div class="wstep"><div class="wstep-num">3</div><div class="wstep-body"><b>Crie sua primeira reserva</b><p>Com os quartos prontos, já pode receber hóspedes.</p><button class="btn btn-primary btn-sm" onclick="navTo(\'#r\')">Ir para Reservas</button></div></div>'+
  '</div>'+
  '<div class="alerts">'+avisoPlanoHTML()+'</div>';
  return;
}

el.innerHTML='<div class="page-header"><div><h2>Painel de Controle</h2><p>Visão geral do hotel'+infoTurno+'</p></div><div class="page-header-actions"><button class="btn btn-primary" onclick="navTo(\'#r\')">Nova Reserva</button><button class="btn btn-secondary" onclick="navTo(\'#ci\')">Check-in Rápido</button></div></div>'+
'<div class="cards-row">'+
'<div class="stat-card"><h3>Ocupação</h3><div class="value">'+ocPercent+'%</div><div class="sub">'+ocupados+'/'+totalQuartos+' quartos</div></div>'+
...
'<div class="stat-card"><h3>Receita do Mês</h3><div class="value">'+fmtC(faturamentoMes)+'</div></div>'+
...
if(entradasVencidas.length>0)alertas+='<div class="alert alert-danger">'+aIco("danger")+'<span><b>'+entradasVencidas.length+' reserva(s) com entrada vencida sem check-in registrado.</b> O hóspede pode ter chegado sem o check-in ser lançado - verifique com o hóspede e regularize. <a onclick="navTo(\'#ci\')" style="color:var(--accent-2);text-decoration:underline;cursor:pointer">Ir para Check-in</a></span></div>';
if(checkoutsHoje.length>0)alertas+='<div class="alert alert-warning">'+aIco("warning")+'<span>'+checkoutsHoje.length+' hóspede(s) precisam fazer check-out hoje.</span></div>';
if(pendentes.length>0)alertas+='<div class="alert alert-info">'+aIco("info")+'<span>'+pendentes.length+' reserva(s) pendente(s) aguardando confirmação.</span></div>';
if(ocupados/totalQuartos>0.8)alertas+='<div class="alert alert-warning">'+aIco("warning")+'<span>Ocupação acima de 80%! Considere verificar disponibilidade.</span></div>';
if(ocupados/totalQuartos<0.3)alertas+='<div class="alert alert-info">'+aIco("info")+'<span>Ocupação abaixo de 30%. Considere promover o hotel.</span></div>';
if(manutencao>0)alertas+='<div class="alert alert-warning">'+aIco("tool")+'<span>'+manutencao+' quarto(s) em manutenção.</span></div>';
...
if(prox.length){el.innerHTML+='<h3 style="margin-bottom:12px;color:var(--text)">Próximas Reservas</h3><table><tr><th>Hóspede</th><th>Quarto</th><th>Check-in</th><th>Check-out</th><th>Status</th></tr>'+
...
'<span class="lg"><i class="dot dot-disponivel"></i>Disponível</span>'+
...
'<span class="lg"><i class="dot dot-manutencao"></i>Manutenção</span>'+
'</div></div>';
...
var label=stt==="ocupado"?"Ocupado":stt==="limpeza"?"Limpeza":stt==="manutencao"?"Manutenção":temReservaFutura?"Reservado":stt==="reservado"?"Reservado":"Livre";
...
(nome?'<div class="qcard-hosp"><span class="qcard-avatar">'+esc(iniciais)+'</span><span class="qcard-nome">'+esc(nome)+'</span></div>':'<div class="qcard-vazio">Sem hóspede</div>')+
infoReserva+
'</div>';}
...
var stt=q.status||"disponivel";
var temReservaFutura=r&&(r.status==="confirmada"||r.status==="pendente")&&(stt==="disponivel"||stt==="limpeza");
var label=temReservaFutura?"Reservado":stt==="disponivel"?"Disponível":stt==="ocupado"?"Ocupado":stt==="reservado"?"Reservado":stt==="limpeza"?"Em limpeza":"Em manutenção";
...
if(hosp){body+='<div class="qmodal-info"><div class="qmodal-row"><span>Hóspede</span><b>'+esc(hosp.nome)+'</b></div>'+
...
'</div>'}else{body+='<p style="color:var(--text-mute);font-size:14px">Este quarto não possui hóspede no momento.</p>'}
